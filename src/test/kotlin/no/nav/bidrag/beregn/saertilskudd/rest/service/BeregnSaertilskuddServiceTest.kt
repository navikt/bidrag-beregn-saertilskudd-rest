...
internal class BeregnSaertilskuddServiceTest {
    ...
    fun settOppSjablonMocks() {
        Mockito.`when`(sjablonConsumerMock?.hentSjablonSjablontall())
            .thenReturn(from(HttpStatus.OK, TestUtil.dummySjablonSjablontallListe()))
        Mockito.`when`(sjablonConsumerMock?.hentSjablonSamvaersfradrag())
            .thenReturn(from(HttpStatus.OK, TestUtil.dummySjablonSamvaersfradragListe()))
        Mockito.`when`(sjablonConsumerMock?.hentSjablonBidragsevne())
            .thenReturn(from(HttpStatus.OK, TestUtil.dummySjablonBidragsevneListe()))
        Mockito.`when`(sjablonConsumerMock?.hentSjablonTrinnvisSkattesats())
    ...
    fun skalHaKorrektSjablonGrunnlagNaarBeregningsmodulenKalles() {
        ...
        Mockito.`when`(bidragsevneCoreMock!!.beregnBidragsevne(bidragsevneGrunnlagTilCoreCaptor.capture()))
            .thenReturn(TestUtil.dummyBidragsevneResultatCore())
        Mockito.`when`(bpAndelSaertilskuddCoreMock!!.beregnBPsAndelSaertilskudd(bpAndelSaertilskuddGrunnlagTilCoreCaptor.capture()))
            .thenReturn(TestUtil.dummyBPsAndelSaertilskuddResultatCore())
        Mockito.`when`(samvaersfradragCoreMock!!.beregnSamvaersfradrag(samvaersfradragGrunnlagTilCoreCaptor.capture()))
            .thenReturn(TestUtil.dummySamvaersfradragResultatCore())
        Mockito.`when`(saertilskuddCoreMock!!.beregnSaertilskudd(saertilskuddGrunnlagTilCoreCaptor.capture()))
            ...
            Executable {
                assertThat(sjablonPeriodeListe.stream()
                    ...
                    .isEqualTo(TestUtil.dummySjablonSjablontallListe().stream()
                        .filter { (typeSjablon): Sjablontall -> typeSjablon == "0017" }
                        .filter { (_, datoFom, datoTom): Sjablontall ->
    ...
    fun skalBeregneSaertilskudd() {
        Mockito.`when`(bidragsevneCoreMock!!.beregnBidragsevne(ArgumentMatchers.any())).thenReturn(TestUtil.dummyBidragsevneResultatCore())
        Mockito.`when`(bpAndelSaertilskuddCoreMock!!.beregnBPsAndelSaertilskudd(ArgumentMatchers.any()))
            .thenReturn(TestUtil.dummyBPsAndelSaertilskuddResultatCore())
        Mockito.`when`(samvaersfradragCoreMock!!.beregnSamvaersfradrag(ArgumentMatchers.any()))
            .thenReturn(TestUtil.dummySamvaersfradragResultatCore())
        Mockito.`when`(saertilskuddCoreMock!!.beregnSaertilskudd(ArgumentMatchers.any())).thenReturn(TestUtil.dummySaertilskuddResultatCore())
        val beregnSaertilskuddResultat = beregnSaertilskuddService!!.beregn(TestUtil.byggTotalSaertilskuddGrunnlag())
        assertAll(
            Executable { assertThat(beregnSaertilskuddResultat.responseEntity.statusCode).isEqualTo(HttpStatus.OK) },
            Executable { assertThat(beregnSaertilskuddResultat.responseEntity.body).isNotNull() },
            Executable { assertThat(beregnSaertilskuddResultat.responseEntity.body?.beregnetSaertilskuddPeriodeListe).isNotNull() },
            Executable { assertThat(beregnSaertilskuddResultat.responseEntity.body?.beregnetSaertilskuddPeriodeListe).hasSize(1) }
        )
    }
    ...
    fun skalKasteUgyldigInputExceptionVedFeilReturFraBidragsevneCore() {
        Mockito.`when`(bidragsevneCoreMock!!.beregnBidragsevne(ArgumentMatchers.any())).thenReturn(TestUtil.dummyBidragsevneResultatCoreMedAvvik())
    ...
    fun skalKasteUgyldigInputExceptionVedFeilReturFraSaertilskuddCore() {
        ...
        Mockito.`when`(samvaersfradragCoreMock!!.beregnSamvaersfradrag(ArgumentMatchers.any()))
            .thenReturn(TestUtil.dummySamvaersfradragResultatCore())
        Mockito.`when`(saertilskuddCoreMock!!.beregnSaertilskudd(ArgumentMatchers.any())).thenReturn(TestUtil.dummySaertilskuddResultatCoreMedAvvik())
        Assertions.assertThatExceptionOfType(UgyldigInputException::class.java)
            .isThrownBy { beregnSaertilskuddService!!.beregn(TestUtil.byggTotalSaertilskuddGrunnlag()) }
